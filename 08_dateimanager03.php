...
function zeigeDateigroesse($pfad) {
	$groesse = filesize($pfad);
	$einheit = ["B","kB","MB","GB","TB"];
	$cnt = 0;
	
	while($groesse/1000>=1 && $cnt<count($einheit)) {
		$groesse = $groesse/1000;
		$cnt++;
	}
	
	return round($groesse,2)." ".$einheit[$cnt];
}
?>
<!doctype html>
<html lang="de">
	...
	<body>
		<form method="post" id="frm" enctype="multipart/form-data">
			<input type="hidden" name="auszulesendesVZ" value="<?php echo($aktuellesVZ); ?>">
			<section>
				<?php
